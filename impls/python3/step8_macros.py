...
def EVAL(ast: MalType, env, is_macro=False) -> MalType:
    while True:
        ...
        if ast[0] == 'defmacro!':
            assert len(ast) == 3, "defmacro! requires two args"
            env.set(ast[1], EVAL(ast[2], env, is_macro=True))
            return env.get(ast[1])
        elif ast[0] == 'let*':
            if len(ast) != 3:
            ...
            a1, a2 = ast[1], ast[2]
            return MalFunction(EVAL=EVAL, ast=a2, env=env, params=a1, is_macro=is_macro)
        else:
            el = eval_ast(ast, env)
            f = el[0]
